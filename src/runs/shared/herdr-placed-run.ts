...
export class HerdrPiSession implements ChildSession {
	...
	prompt(text: string) { return this.#operation("prompt", text); } steer(text: string) { return this.#operation("steer", text); } followUp(text: string) { return this.#operation("follow-up", text); }
	async switchModel(_model: string): Promise<void> { throw new Error("Pane-native sessions do not support live account-alias model switching."); }
	async abort() { this.#aborted = true; await this.#operation("abort"); }
