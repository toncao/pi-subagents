...
interface ForkContextResolverOptions {
	 ...
	 * thinking. Defaults to false: Pi >= 0.85.0 recovers from signed-thinking mismatches
	 * on the transport, so sanitization alone is not a reason to turn thinking off.
	 * The executor passes an explicit gate so older Pi runtimes can stay conservative. */
